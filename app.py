...
GM_DICTIONARY = {
    "G00": "급속 이송", "G01": "직선 절삭 이송", "G02": "원호 가공 CW", "G03": "원호 가공 CCW",
    "G04": "일시 정지 / Dwell", "G28": "기계 원점 복귀", "G40": "인선 보정 취소",
    "G41": "인선 보정 좌측", "G42": "인선 보정 우측", "G43": "공구 길이 보정 +",
    "G49": "공구 길이 보정 취소", "G54": "공작물 좌표계 1", "G80": "사이클 취소",
    "G81": "스폿 드릴링", "G83": "심공 드릴링", "G84": "태핑 사이클",
    "M00": "프로그램 정지", "M01": "선택적 정지", "M02": "프로그램 종료",
    "M03": "주축 정회전", "M04": "주축 역회전", "M05": "주축 정지",
    "M06": "공구 교환", "M08": "절삭유 ON", "M09": "절삭유 OFF", "M30": "프로그램 종료"
}
...
def load_data():
    ...
    try:
        with open(DB_FILE, "r", encoding="utf-8") as f:
            ...
            if not content: raise ValueError("File is empty")
    ...
    if os.path.exists(파일이름):
        try:
            with open(파일이름, 'r', encoding='utf-8') as f:
                데이터 = json.load(f)
                # 빈 파일이 남아있다면 강제로 원본 데이터 주입
                if (not 데이터 or len(데이터) == 0) and 파일이름 == "원영공구_장부.json":
